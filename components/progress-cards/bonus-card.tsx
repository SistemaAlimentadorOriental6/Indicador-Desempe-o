...
import { BonusMonthlyChart } from "../chart-components"

import { useBonificaciones, useDatosDisponibles } from "./hooks"
import { formatearMoneda, obtenerBonoBaseAnual } from "./utils"

interface PropsTarjetaBonificaciones {
    userCode: string
}

const TarjetaBonificacionesBase: React.FC<PropsTarjetaBonificaciones> = ({ userCode }) => {
    ...
    const { data, estaCargando, error, recargar } = useBonificaciones(
        userCode,
    ...
    const datosMensualesAnio = useMemo(() => {
        if (!data?.monthlyBonusData || !anioSeleccionado) return []
        return data.monthlyBonusData
            .filter((item: any) => item.year === anioSeleccionado)
            ...
            <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <CardTitle className="text-base font-semibold text-green-700">Bonificaciones</CardTitle>
            ...
            <CardContent className="space-y-4">
                ...
                {anioSeleccionado && datosMensualesAnio.length > 0 && (
                    <div>
                        ...
                        <BonusMonthlyChart data={datosMensualesAnio} year={anioSeleccionado} isLoading={false} />
                    </div>
                )}

                <div className="text-center space-y-2">
                    <div className="flex items-center justify-between">
                        <span className="text-xs text-green-700">Eficiencia</span>
...
export const TarjetaBonificaciones = memo(TarjetaBonificacionesBase)
